...
The system MUST run `composer require drupal/core-recommended:^<target> drupal/core:^<target> --update-with-dependencies` after modifying composer.json.

#### Scenario: Composer update succeeds

- GIVEN a valid composer.json with updated constraint
- WHEN composer execution runs
- THEN it MUST invoke composer and propagate its exit code
...
The system SHOULD create a backup of `composer.json` before modifying it.

#### Scenario: Backup created

- GIVEN a valid composer.json
- WHEN `drup upgrade-core` modifies it
- THEN a backup file `composer.json.bak` MUST exist with the original content
