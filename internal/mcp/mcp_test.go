...
func TestServer_ListTools(t *testing.T) {
	...
	if len(tools) != 36 {
		t.Errorf("len(tools) = %d, want 36", len(tools))
	}
}
...
func TestServer_PostWireUpCountIs31(t *testing.T) {
	...
	if len(tools) != 40 {
		t.Errorf("post-wire-up tool count = %d, want 40 (36 default + 4 backup)", len(tools))
	}
}
...
func TestServer_ToolCount(t *testing.T) {
	var buf bytes.Buffer
	server := NewServer(&buf, "test")
	if got := server.ToolCount(); got != 36 {
		t.Errorf("default ToolCount() = %d, want 36", got)
	}
	...
	if got := server.ToolCount(); got != 38 {
		t.Errorf("after adding 2 tools, ToolCount() = %d, want 38", got)
	}
}
