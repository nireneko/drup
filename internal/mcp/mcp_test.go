...
func TestServer_ListTools(t *testing.T) {
	...
	if len(tools) != 39 {
		t.Errorf("len(tools) = %d, want 39", len(tools))
	}
}
...
func TestServer_PostWireUpCountIs43(t *testing.T) {
	...
	if len(tools) != 43 {
		t.Errorf("post-wire-up tool count = %d, want 43 (39 default + 4 backup)", len(tools))
	}
}
...
func TestServer_ToolCount(t *testing.T) {
	...
	if got := server.ToolCount(); got != 39 {
		t.Errorf("default ToolCount() = %d, want 39", got)
	}
	...
	if got := server.ToolCount(); got != 41 {
		t.Errorf("after adding 2 tools, ToolCount() = %d, want 41", got)
	}
}
