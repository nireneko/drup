...
func TestServer_ListTools(t *testing.T) {
	...
	if len(tools) != 23 {
		t.Errorf("len(tools) = %d, want 23", len(tools))
	}
}
...
// TestServer_PostWireUpCountIs27 asserts that after the production-style
...
func TestServer_PostWireUpCountIs27(t *testing.T) {
	req := JSONRPCRequest{
		JSONRPC: "2.0",
		ID:      1,
		Method:  "tools/list",
	...
	if len(tools) != 27 {
		t.Errorf("post-wire-up tool count = %d, want 27 (23 default + 4 backup)", len(tools))
	}
}
